...
{
	if (gameObject != nullptr && QT_Box->Intersects(gameObject->global_AABB))
	{
...
void Quadtree::Split()
{
	...
	//second Child
	float3 secondMin = QT_Box->minPoint + float3(x / 2, 0.0f, 0.0f);
	AABB second(secondMin, secondMin + diagonal);
	children[1] = new Quadtree(second, level + 1);

	//third Child
	float3 thirdMin = QT_Box->minPoint + float3(0.0f, 0.0f, z / 2);
